...
static void pmm_init() {
    ...
    // round up
    uintptr_t chunks_size = (pmsize + PAGESIZE - 1) / PAGESIZE;
    printf("chunks: [0x%x, 0x%x), chunks_size: 0x%x(%d)\n", 
           (uint64_t)(uintptr_t)chunks, 
           (uint64_t)(uintptr_t)(chunks + chunks_size),
           (uintptr_t)chunks_size, (uint64_t)chunks_size);
